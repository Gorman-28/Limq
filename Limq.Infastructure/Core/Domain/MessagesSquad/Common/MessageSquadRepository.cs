...
public class MessageSquadRepository : IMessageSquadRepository
{
    ...
    public async Task<MessageSquad> Find(Guid squadId, Guid userId)
    {
        var messageSquad = await _limqDbContext.MessagesSquad.FirstOrDefaultAsync(ms => ms.SquadId == squadId && ms.UserFromId == userId);
        return messageSquad;
    }

    public async Task<Unit> Remove(Guid squadId, Guid userId)
    {
        var messageSquad = await _limqDbContext.MessagesSquad.FirstOrDefaultAsync(ms => ms.SquadId == squadId && ms.UserFromId == userId);
